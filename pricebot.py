...
ADMIN_ID = int(os.getenv("ADMIN_ID"))  # Your admin Telegram ID here
...
def get_price(symbol):
    try:
        ...
        # fallback for coins with prefix "1000"
        ...
        if long_match or short_match:
            ...
            if not price:
                await context.bot.send_message(
                    ...
                    text=f"⚠️ Could not fetch price for '{coin_id}'. Please check symbol."
                )
                return
            ...
            # Spot message ONLY for Buy signals
            if not is_short:
                spot_msg = (
                    f"Spot Buy\n\n"
                    f"{symbol_pair}\n\n"
